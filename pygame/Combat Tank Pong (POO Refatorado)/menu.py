import pygame
from pygame import JOYBUTTONDOWN, JOYHATMOTION
...
while config.stage_select < 0:
    ...
    for event in pygame.event.get():
        ...
        if event.type == JOYBUTTONDOWN:
            if event.button == pygame.joystick.Joystick(0).get_button(1):
                config.stage_select = stage - 1

        if event.type == JOYHATMOTION:
            if pygame.joystick.Joystick(0).get_hat(0) == (0, 1) or pygame.joystick.Joystick(0).get_hat(0) == (1, 0):
                if stage == 2:
                    stage = 1
                else:
                    stage += 1
